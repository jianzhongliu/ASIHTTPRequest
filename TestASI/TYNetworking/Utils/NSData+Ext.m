...
{
    NSMutableData *rst = [[NSMutableData alloc]initWithData:data];
    [rst rc4TransformWithKey:key];
    return rst;
}
...
{
    NSMutableData *rst = [[NSMutableData alloc]initWithData:data];
    [rst rc4TransformWithKeyString:key];
    return rst;
}
...
- (NSString *)hexStringValue
{
    ...
    *hex = 0;
    NSString *hexStr = [NSString stringWithUTF8String:strbuf];
    free(strbuf);
    return hexStr;
}
...
- (NSString *)base64EncodedString
{
	...
	NSString *rst = [[NSString alloc]initWithBytes:outputBuffer
                                            length:outputLength
...
- (NSString *)md5Base64
{
    ...
    CC_MD5(original_str, [self length], md5);
    
    size_t lenBase64;
	char *bufBase64 = NewBase64Encode(md5, CC_MD5_DIGEST_LENGTH, true, &lenBase64);
    
    NSString *rst = [[NSString alloc] initWithBytes:bufBase64 length:lenBase64
                                           encoding:NSASCIIStringEncoding];
	free(bufBase64);
    return rst;
}
...
-(void)appendBase64String:(NSString *)str
{
	...
	size_t outputLength;
	void *outputBuffer = NewBase64Decode([data bytes], [data length], &outputLength);
	[self appendBytes:outputBuffer length:outputLength];
	free(outputBuffer);
}
...
-(void)rc4TransformWithKey:(NSData *)key
{
    ...
    int lenDat = [self length];
    
    const void *bufKey = [key bytes];
    rc4_key rc4Key;
    
    //数据
    void *rstBuf = (void *)[self bytes];
    
    //准备key
    prepare_key(bufKey, lenKey, &rc4Key);
    
    //数据处理
    rc4(rstBuf, lenDat, &rc4Key); 
}

-(void)rc4TransformWithKeyString:(NSString *)key
{
    [self rc4TransformWithKey:[key dataUsingEncoding:NSUTF8StringEncoding]];
}
@end
